...
function showMenu(e, index, row) {
    //选中这个行
    $("#permissionGrid").datagrid("selectRow", index);
    //第0个位置的面板不支持相应功能
    e.preventDefault();
    $('#gridMenu').menu('show', {
        left: e.pageX,
...
$(function () {
    ...
    let permissionDialog = $("#permissionDialog");
    ...
    mymethod = {
        ...
        del() {//多行删除
            /*let row = permissionGrid.datagrid("getSelected");*/
            let rows = permissionGrid.datagrid("getSelections");
            //如果用户没有选中行
            if (!rows) {
                ...
                $.messager.confirm('确认', `您确认想要删除这${rows.length}条记录吗？`, function (r) {
                    if (r) {
                        for (let i = 0; i < rows.length; i++) {
                            $.get('/permission/delete', {id: rows[i].id}, function (result) {
                                if (result.success) {
        ...
        save() {
            ...
            if ($permissionId) {
                url = "/permission/update?cmd=_update";
            }
        ...
        search() {
            ...
            permissionGrid.datagrid("load", params);/*重新加载数据（重新发送请求）*/
        },
        close() {
    ...
    permissionGrid.datagrid({
        onDblClickCell:function (index, field, value) {
            permissionGrid.datagrid('beginEdit', index);
            let ed = permissionGrid.datagrid('getEditor', {index:index,field:field});
            $(ed.target).focus();
        }
        /*//双击行的时候修改当前条数据
