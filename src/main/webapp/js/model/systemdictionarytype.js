...
function showMenu(e, index, row) {
    //选中这个行
    $("#systemdictionarytypeGrid").datagrid("selectRow", index);
    //第0个位置的面板不支持相应功能
    e.preventDefault();
    $('#gridMenu').menu('show', {
        left: e.pageX,
...
$(function () {
    ...
    /*拿到常用的组件*/
    let systemdictionarytypeGrid = $("#systemdictionarytypeGrid");
    let searchForm = $("#searchForm");
    let systemdictionarytypeDialog = $("#systemdictionarytypeDialog");
    ...
    mymethod = {
        add() {
            ...
            //先清空
            editForm.form("clear");
            systemdictionarytypeDialog.dialog("center").dialog("open");
        },
        del() {//多行删除
            /*let row = systemdictionarytypeGrid.datagrid("getSelected");*/
            let rows = systemdictionarytypeGrid.datagrid("getSelections");
            //如果用户没有选中行
            if (!rows) {
                ...
                $.messager.confirm('确认', `您确认想要删除这${rows.length}条记录吗？`, function (r) {
                    if (r) {
                        for (let i = 0; i < rows.length; i++) {
                            $.get('/systemdictionarytype/delete', {id: rows[i].id}, function (result) {
                                if (result.success) {
                                    systemdictionarytypeGrid.datagrid("reload");
                                } else {
        ...
        save() {
            let url = "/systemdictionarytype/save";
            let $systemdictionarytypeId = $("#systemdictionarytypeId").val();
            if ($systemdictionarytypeId) {
                url = "/systemdictionarytype/update?cmd=_update";
            }
        ...
        search() {
            ...
            systemdictionarytypeGrid.datagrid("load", params);/*重新加载数据（重新发送请求）*/
        },
        close() {
     ...
     */
    systemdictionarytypeGrid.datagrid({
        onDblClickCell:function (index, field, value) {
            systemdictionarytypeGrid.datagrid('beginEdit', index);
            let ed = systemdictionarytypeGrid.datagrid('getEditor', {index:index,field:field});
            $(ed.target).focus();
        }
        /*//双击行的时候修改当前条数据
