...
$(function () {
    ...
    mymethod = {
        ...
        del() {//多行删除
            ...
            let rows = menuGrid.datagrid("getSelections");
            //如果用户没有选中行
            if (!rows) {
                ...
                $.messager.confirm('确认', `您确认想要删除这${rows.length}条记录吗？`, function (r) {
                    if (r) {
                        for (let i = 0; i < rows.length; i++) {
                            $.get('/menu/delete', {id: rows[i].id}, function (result) {
                                if (result.success) {
        ...
        search() {
            ...
            menuGrid.datagrid("load", params);/*重新加载数据（重新发送请求）*/
        },
        close() {
    ...
    menuGrid.datagrid({
        onDblClickCell:function (index, field, value) {
            menuGrid.datagrid('beginEdit', index);
            let ed = menuGrid.datagrid('getEditor', {index:index,field:field});
            $(ed.target).focus();
        }
        /*//双击行的时候修改当前条数据
