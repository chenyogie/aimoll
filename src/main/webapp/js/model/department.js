...
function showMenu(e, index, row) {
    //选中这个行
    $("#departmentGrid").datagrid("selectRow", index);
    //第0个位置的面板不支持相应功能
    e.preventDefault();
    $('#gridMenu').menu('show', {
        left: e.pageX,
...
$(function () {
    ...
    /*拿到常用的组件*/
    let departmentGrid = $("#departmentGrid");
    let searchForm = $("#searchForm");
    let departmentDialog = $("#departmentDialog");
    ...
    mymethod = {
        ...
        del() {//多行删除
            /*let row = departmentGrid.datagrid("getSelected");*/
            let rows = departmentGrid.datagrid("getSelections");
            //如果用户没有选中行
            if (!rows) {
                ...
                $.messager.confirm('确认', `您确认想要删除这${rows.length}条记录吗？`, function (r) {
                    if (r) {
                        for (let i = 0; i < rows.length; i++) {
                            $.get('/department/delete', {id: rows[i].id}, function (result) {
                                if (result.success) {
        ...
        save() {
            ...
            if ($departmentId) {
                url = "/department/update?cmd=_update";
            }
        ...
        search() {
            ...
            departmentGrid.datagrid("load", params);/*重新加载数据（重新发送请求）*/
        },
        close() {
    ...
    departmentGrid.datagrid({
        onDblClickCell:function (index, field, value) {
            departmentGrid.datagrid('beginEdit', index);
            let ed = departmentGrid.datagrid('getEditor', {index:index,field:field});
            $(ed.target).focus();
        }
        /*//双击行的时候修改当前条数据
