...
function showMenu(e, index, row) {
    //选中这个行
    $("#systemdictionarydetailGrid").datagrid("selectRow", index);
    //第0个位置的面板不支持相应功能
    e.preventDefault();
    $('#gridMenu').menu('show', {
        left: e.pageX,
...
$(function () {
    ...
    /*拿到常用的组件*/
    let systemdictionarydetailGrid = $("#systemdictionarydetailGrid");
    let searchForm = $("#searchForm");
    let systemdictionarydetailDialog = $("#systemdictionarydetailDialog");
    ...
    mymethod = {
        add() {
            ...
            //先清空
            editForm.form("clear");
            systemdictionarydetailDialog.dialog("center").dialog("open");
        },
        del() {//多行删除
            /*let row = systemdictionarydetailGrid.datagrid("getSelected");*/
            let rows = systemdictionarydetailGrid.datagrid("getSelections");
            //如果用户没有选中行
            if (!rows) {
                ...
                $.messager.confirm('确认', `您确认想要删除这${rows.length}条记录吗？`, function (r) {
                    if (r) {
                        for (let i = 0; i < rows.length; i++) {
                            $.get('/systemdictionarydetail/delete', {id: rows[i].id}, function (result) {
                                if (result.success) {
                                    systemdictionarydetailGrid.datagrid("reload");
                                } else {
        ...
        save() {
            let url = "/systemdictionarydetail/save";
            let $systemdictionarydetailId = $("#systemdictionarydetailId").val();
            if ($systemdictionarydetailId) {
                url = "/systemdictionarydetail/update?cmd=_update";
            }
        ...
        search() {
            ...
            systemdictionarydetailGrid.datagrid("load", params);/*重新加载数据（重新发送请求）*/
        },
        close() {
            systemdictionarydetailDialog.dialog("close");
        }
    };
     ...
     */
    systemdictionarydetailGrid.datagrid({
        onDblClickCell:function (index, field, value) {
            systemdictionarydetailGrid.datagrid('beginEdit', index);
            let ed = systemdictionarydetailGrid.datagrid('getEditor', {index:index,field:field});
            $(ed.target).focus();
        }
        /*//双击行的时候修改当前条数据
